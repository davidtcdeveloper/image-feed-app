...
The Unsplash Image Feed App leverages native platform UI toolkits (Jetpack Compose on Android and SwiftUI on iOS) bound to a shared multiplatform business/presentation layer. The application fully implements the public (unauthenticated) features of the Unsplash API across 5 key phases:
...
1. Open **Xcode**.
2. Create a new Xcode project or workspace configured as a SwiftUI App inside `iosApp/`.
3. Drag the Swift source files located under `iosApp/iosApp` into your project navigator.
4. Link the generated `:shared` framework target using Xcode Build Phase dependencies.
5. Hit **Run** inside Xcode.
